...
// Increase default icon size and make the 3D object slightly larger/closer
export default function ServiceIcon3D({ size = 160, type = 'box' }) {
  ...
  function Icon() {
    ...
    return (
      <Float speed={1} floatIntensity={0.9} rotationIntensity={0.4}>
        <mesh ref={ref} scale={[1.35, 1.35, 1.35]}>
          {type === 'torus' ? (
            // slightly larger torus for better visibility
            <torusGeometry args={[0.7, 0.16, 12, 64]} />
          ) : (
            // larger icosahedron
  ...
  return (
    <div style={{ width: size, height: size }} aria-hidden>
      {/* camera moved closer to zoom in so icon appears larger */}
      <Canvas camera={{ position: [0, 0, 2.6], fov: 50 }} style={{ width: '100%', height: '100%' }} dpr={[1, 1.4]}>
